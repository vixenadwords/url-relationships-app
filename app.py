...
if uploaded_file is not None:
    try:
        ...
        embedding_lengths = {
            ...
            'GPT-2 (768)': 768,
            'GPT-3 (2048)': 2048,
            'OpenAI Embeddings (1536)': 1536,
            'Sentence Transformers (384)': 384,
            'Sentence Transformers (768)': 768,
            'Sentence Transformers (1024)': 1024,
            'Custom': 'Custom'
        }

        # Allow the user to select the embedding length or input a custom value
        embedding_length_choice = st.selectbox("Select the embedding length", list(embedding_lengths.keys()))
        if embedding_lengths[embedding_length_choice] == 'Custom':
            expected_embedding_length = st.number_input('Enter the expected embedding length', min_value=1, value=1536)
        else:
            expected_embedding_length = embedding_lengths[embedding_length_choice]
